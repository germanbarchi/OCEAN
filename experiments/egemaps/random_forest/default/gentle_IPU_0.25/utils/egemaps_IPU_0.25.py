...
def concat_df(df_ipu_25,labels_df):

    # Concat features and label dataframes
  
    df_ipu_25['Name']=df_ipu_25['Name'].apply(lambda x:x.replace('.wav',''))
    labels_df['audio_tag']=labels_df['audio_tag'].apply(lambda x: x.replace('.mp4',''))

    for i in range(df_ipu_25.shape[0]):
        split_=df_ipu_25.loc[i,'Name'].split('_')

        if len(split_)==2:
            df_ipu_25.loc[i,'videoid']= split_[0]

        else:
            df_ipu_25.loc[i,'videoid']='_'.join(split_[:-1])

    
    final_df=pd.merge(df_ipu_25,labels_df,left_on='videoid',right_on='audio_tag')
    final_df=final_df.loc[:,~final_df.columns.isin(['videoid','Name','Part'])]
